...
public class UpdateDBThread_NoPreview extends Thread {
	...
	public UpdateDBThread_NoPreview(Main app, ShutterProvider provider, SQLManager sqlManager) {
		super();
	...
	public void run() {
		...
		while(!isInterrupted() && !isStop){
			...
			page_number++;
			if (total*2<per_page*page_number)
				break;
			}
			catch (IOException e) {
